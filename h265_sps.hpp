#ifndef __H265_SPS_HPP__
#define __H265_SPS_HPP__
// copy from https://github.com/latelee/H264BSAnalyzer/blob/master/H264BSAnalyzer/h265_stream.cpp
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
#include <vector>
...
static inline uint32_t bs_byte_aligned(bs_t* b) { return (b->bits_left == 8); }
...
#define bs_print_state(b)                                                                          \
    fprintf(stderr,                                                                                \
            "%s:%d@%s: b->p=0x%02hhX, b->left = %d\n",                                             \
            __FILE__,                                                                              \
            __LINE__,                                                                              \
            __FUNCTION__,                                                                          \
            *b->p,                                                                                 \
...
/**
   ...
   The reason why they are all contained in one place is that some of them depend on others, we need
   to have all of them available to read or write correctly.
 */
typedef struct
{
    h265_nal_t* nal;
